            ...
            files.map(file =>
              db.getSessionBySession(session)
                .then(({ id }) =>
                  createItem({
                    sessionId: id,
                    path: path.join(config.directories.recordings, session, file),
                  }))
              ));
...
module.exports = {
  ...
  upload: (uploadStatuses) => {
    ...
    const sensorData = db.getSensorDataByStatus({ statuses })
      .then((rows) => rows.reduce((result, row) => {
        const res = Object.assign({}, result);
        if (!res[row.type]) {
          res[row.type] = {};
        }
        if (!res[row.type][row.sessionId]) {
          res[row.type][row.sessionId] = [];
        }
        res[row.type][row.sessionId].push(row);
        log(row.type, row.sessionId, res[row.type][row.sessionId]);
        return res;
      }, {}))
      .then((data) =>
        Object.keys(data).map((type) =>
          Object.keys(data[type]).map((sessionId) =>
            uploadSensorData(data[type][sessionId]))));

    const files = getOrphanRecordingsFiles()
      .then(() =>
        db.getFilesByStatus({ statuses }))
      .then((rows) =>
        Promise.all(rows.map(upload)));

    return Promise.all([sensorData, files]);
  },
};
