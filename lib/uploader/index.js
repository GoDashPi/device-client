...
const createItem = (item) =>
  stat(item.path)
    .then((stats) => {
      ...
      // const created = parseInt(moment(stats.birthtime).format('x'), 10);
      const created = moment(stats.birthtime).utc().format('YYYY-MM-DD HH:mm:ss.SSS');
      const statusItem =
        Object.assign({ status: fileStatus.RECORDING, created }, item);
      return db.insertItem(statusItem)
        .then(() => statusItem);
    })
