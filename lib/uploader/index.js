      ...
      db.changeStatus({
        ...
        .then((session) => {
          ...
          return fetch(`${process.env.DASHPI_API}/register-chunk`, params)
            ...
            .then(({ url }) => ({ url, id: row.id, filename: row.path, type: row.type }));
        })
        .then(uploadFile)
        .catch((error) => {
        ...
        .then((orphans) => {
          const orphansInsert =
            orphans.map(({ id, files }) =>
              files.map(file => {
                const filePath = path.join(config.recordingsDirectory, id, file);
                log('insert', filePath);
                return getCreatedTime(filePath)
                  .then(created => db.insertIntoFiles({
                    id,
                    path: filePath,
                    status: fileStatus.READY_FOR_UPLOAD,
                    created,
