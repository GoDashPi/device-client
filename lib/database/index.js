...
const insertIntoFiles = ({ id, path, status, created, type }) => new Promise((resolve, reject) => {
  db.run(`INSERT INTO files (session, path, status, created, type) 
           VALUES ('${id}', '${path}', '${status}', '${created}', '${type}')`,
    (error) => {
      if (error) {
        logError(error);
        return reject(error);
