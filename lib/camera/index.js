...
const getRecordingProcess = (session) => {
  ...
  if (process.env.DASHPI_ENV === 'device') {
    command = 'raspivid';
    filename = path.join(session.recordingsPath, '%06d.h264');
    params = `${initialParams} -o`;
  } else if (process.env.DASHPI_ENV === 'development') {
    command = 'ffmpeg';
    filename = path.join(session.recordingsPath, '%06d.mkv');
...
const run = ({ id, recordingsPath }) => {
  const recordingProcess = getRecordingProcess({ id, recordingsPath });
